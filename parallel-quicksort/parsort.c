...
int main( int argc, char **argv ) {
  ...
  if ( file_size % sizeof(int64_t) != 0 ) {
    fprintf( stderr, "Error: file size (%lu) is not a multiple of %zu\n",
             file_size, sizeof(int64_t) );
    close( fd );
    exit( 1 );
  }
...
int quicksort( int64_t *arr, unsigned long start, unsigned long end, unsigned long par_threshold ) {
  ...
  // Recursively sort the left and right partitions in child processes
  ...
  // Wait for left child when created
  if ( left_pid > 0 ) {
    int wstatus;
  ...
  // Wait for right child when created
  if ( right_pid > 0 ) {
    int wstatus;
    if ( waitpid( right_pid, &wstatus, 0 ) < 0 ) {
      perror( "waitpid (right)" );
      success = 0;
    } else {
      if ( !WIFEXITED( wstatus ) || WEXITSTATUS( wstatus ) != 0 ) {
        success = 0;
